        ...
        function renderData(targetElementId, data) {
            ...
            if (targetElementId === "tags-results") {
                ...
                data.forEach(item => {
                    const label = document.createElement('label');
                    label.innerHTML = `<input type="checkbox" name="categories" value="${item.id}" class="category-checkbox"> ${item.name}`;
                    targetElement.appendChild(label);
                    targetElement.appendChild(document.createElement('br'));
                });
        ...
        function triggerTagsEndpoint() {
            ...
            const endDate = document.getElementById("end_date").value;
            const selectedCountry = document.querySelector('input[name="country"]:checked')?.value;
            const tagsUrl = `/summarize_news/tags/?country=${selectedCountry}&start_date=${startDate}&end_date=${endDate}`;
            ...
            fetchData(tagsUrl, "tags-results");
